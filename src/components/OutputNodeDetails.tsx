...
interface OutputNodeDetailsProps {
  data: any;
}

const OutputNodeDetails: React.FC<OutputNodeDetailsProps> = ({ data }) => {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div>
        <label style={{ fontSize: 12, color: "#888" }}>Filter Code</label>
        <div style={{ fontSize: 13, marginTop: 2, color: "#444" }}>
          {data?.filterCode || "67c0cabf01bdfbcf5bec657b86398dce"}
        </div>
      </div>
      <LabeledInput label="Name" value={data?.name || "Write"} />
      <LabeledInput
        label="Description"
